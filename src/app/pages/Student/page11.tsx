...
export function Page11({ createLink, useLockableLink }: PageProps) {
  ...
  const question1: QuestionProps = CreateQuestionProps(
    ...
    ["To sort data in ascending order", "To find a specific data point within a larger dataset", "To compress data for storage", "To encrypt information for security",],
    [1]);
  const question2: QuestionProps = CreateQuestionProps(
    "How does binary search work?",
    ["Checks each element one by one until it finds the target", "Divides the list in half and searches the relevant half repeatedly", "Jumps fixed steps through the list to find the target", "Estimates the position of the target using a formula",],
    [1]);
  const question3: QuestionProps = CreateQuestionProps(
    "What is a disadvantage of linear search?",
    ["It only works on sorted data", "It is complex to implement", "It can be very slow for large datasets", "It can skip over the target data",],
    [2]);
  const question4: QuestionProps = CreateQuestionProps(
    "Which statement is true about binary search?",
    ["It works efficiently on both sorted and unsorted data", "It is slower than linear search for large datasets", "It requires the dataset to be sorted", "It checks every element sequentially",],
    [2]);
  const question5: QuestionProps = CreateQuestionProps(
    ...
    ["Stops immediately", "Performs binary search in that block", "Performs linear search within that block", "Jumps again by a larger step",],
    [2]);
  const question6: QuestionProps = CreateQuestionProps(
    ...
    ["Linear Search", "Jump Search", "Binary Search", "Interpolation Search",],
    [3]);
  return (
    <div>
